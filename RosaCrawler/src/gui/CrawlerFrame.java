...
//		.add(new JScrollPane(textResourceRequest), "Resource Request");
	...
	p.setBegin((currentPage - 1) * RosaCrawlerConstants.SEARCH_PAGE_SIZE);
	p.setCount(RosaCrawlerConstants.SEARCH_PAGE_SIZE);
	...
	try
	{
		...
		totalPages = (int) Math.ceil(total / (double) RosaCrawlerConstants.SEARCH_PAGE_SIZE);
	...
	lblInfo.setText("Keyword: " + parameter.getKeyword()
		+ ", hits number: " + result.getHistNum() + ", " + RosaCrawlerConstants.SEARCH_PAGE_SIZE
