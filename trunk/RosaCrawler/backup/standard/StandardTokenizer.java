...
import analysis.LetterDigitBreakTokenizer;
 ...
 * your project and maintaining your own grammar-based tokenizer. <a
 * name="version"/>
 * <p>
...
public final class StandardTokenizer extends Tokenizer
{
	...
	tokensList = new LinkedList<State>();
    }
	...
	{
	    this.restoreState(tokensList.remove());
	    return true;
	}
	while (true)
	{
		...
		if (tokenType == StandardTokenizerImpl.ACRONYM_DEP)
		{
			...
			termAtt.setTermLength(termAtt.termLength() - 1); // remove
									 // extra
									 // '.'
		    }
		    else
		...
		/**
		 * Added by Weiwei Wang
		 */
		if (tokenType == StandardTokenizerImpl.HOST
			|| tokenType == StandardTokenizerImpl.NUM
			|| tokenType == StandardTokenizerImpl.ALPHANUM)
		{

		    Tokenizer lt = new LetterDigitBreakTokenizer(new StringReader(termAtt.term()),correctOffset(offsetAtt.startOffset()));
		    while(lt.incrementToken())
		    {	
			tokensList.add(lt.captureState());
		    }
		}
		if(tokensList.size()>0)
		    this.restoreState(tokensList.remove());
