...
public class FtpCrawler
{
	...
	for (FTPFile file : files)
	{
	    ...
	    {
//		CrawlerLogger.logger.info("entering "+info.getPath());
	...
	ftpSite.setServer("202.119.63.123");
//	ftpSite.setRecursive(FtpSiteInfo.MANUAL_RECURSIVE);
//	ftpSite.setUsername("test");
//	ftpSite.setPassword("test");
	FtpCrawler ftp = new FtpCrawler(ftpSite);
	FtpFileInfoPool pool = new FtpFileInfoPool("test.txt");
	ftp.crawl(pool);
	pool.close();
