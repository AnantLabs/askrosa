...
import utils.Tools;
import analysis.FtpFilePathAnalyzer;
...
public class FtpSearch
{
    ...
    public static SearchResult search(SearchParameter parameter) throws IOException,
	    ParseException, NotBoundException
    {
     ...
     */
    public static SearchResult search(SearchParameter parameter, boolean filterKeyword,
	    boolean doStatistics) throws IOException, ParseException, NotBoundException
    {
	final int N = RosaCrawlerConstants.PAGE_SIZE * 10;
	long begin = System.currentTimeMillis();
	ParallelMultiSearcher searcher = new ParallelMultiSearcher(getAllRemoteSearchers());
	QueryParser qp = new QueryParser(parameter.getField(), getAnalyzer());
...
//	System.out.println("FilteredKeyword:"+keyword);
	...
	RangeQuery updateTimeRange = new RangeQuery(new Term("updatetime", fromTime), null, true);
    ...
    private static SearchResult getSearchResult(Searcher searcher, TopDocs topFieldDocs,
	    SearchParameter parameter, Highlighter h)
    {
	...
	if (value != null && !value.equalsIgnoreCase("null"))
	{
		...
		{ SortField.FIELD_SCORE, new SortField(fieldName, dataType, desc) };
	...
	analyzer.addAnalyzer("path", new FtpFilePathAnalyzer());
	analyzer.addAnalyzer("name", new FtpFilePathAnalyzer());
	return analyzer;
    }
	...
	parameter.setCount(30);
	parameter.setKeyword("friends");
	parameter.setField("name");
	parameter.setSortType("sizedesc");
	while (true)
	{
