...
public class IndexBuilder
{
	...
	// open writers
	indexBackDirectory = FSDirectory.getDirectory(INDEX_BACK);
	fsWriter = new IndexWriter(indexBackDirectory, getAnalyzer(),
		!IndexReader.indexExists(indexBackDirectory),
		IndexWriter.MaxFieldLength.UNLIMITED);
	fsWriter.setRAMBufferSizeMB(RAM_BUFFER);
...
//    }
	...
	try
	{
	    ...
	    FSDirectory indexDir = FSDirectory.getDirectory(INDEX);
	    if (!IndexReader.indexExists(indexDir))
		return;

	    reader = IndexReader.open(indexBackDirectory);
	    ...
	    IndexWriter writer = new IndexWriter(indexBackDirectory,
	...
	try
	{
	    ...
	    FSDirectory back = FSDirectory.getDirectory(INDEX_BACK);
	    FSDirectory index = FSDirectory.getDirectory(INDEX);
    ...
    {
	return new FtpFilePathAnalyzer();
    }
...
//		}
	...
	public boolean pool(FTPFileInfo obj)
	{
		...
		try
		{
		    ...
		    // if (ramDir.sizeInBytes() >= UsefulConstants.MEGA * 50)
		    // mergeIndex();
		}
		catch (CorruptIndexException e)
		{
